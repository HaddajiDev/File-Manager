...
## Overview
This API allows you to upload, download, list, and delete files using MongoDB's GridFS, as well as manage text entries. All requests require a password for authentication.
...
1. **List All Files**
    ...
    - **Curl Command**:
      ```bash
      curl "http://localhost:5000/files/all?password=your_secret_password"
      ```
    - **Response Example**:
      ```
...
2. **Upload a File**
    ...
    - **Curl Command**:
      ```bash
      curl -X POST -F "file=@/path/to/your/file.txt" "http://localhost:5000/files/upload?password=your_secret_password"
      ```
    - **Response**: 
      ```json
...
3. **Download a File by ID**
    ...
    - **Curl Command**:
      ```bash
      curl -o downloaded_file.txt "http://localhost:5000/files/download/<fileId>?password=your_secret_password"
      ```
...
4. **Delete a File by ID**
    ...
    - **Curl Command**:
      ```bash
      curl -X DELETE "http://localhost:5000/files/delete/<fileId>?password=your_secret_password"
      ```
    - **Response**:
      ```json
        File deleted
...
1. **Upload Text**
    ...
    - **Curl Command**:
      ```bash
      curl -d "text=Your text here" "http://localhost:5000/texts/upload?password=your_secret_password"
      ```
    - **Response**: 
      ```json
        Done
...
2. **Get All Texts**
    ...
    - **Curl Command**:
      ```bash
      curl "http://localhost:5000/texts/all?password=your_secret_password"
      ```
    - **Response Example**:
      ```
...
3. **Delete a Text by Index**
    ...
    - **Curl Command**:
      ```bash
      curl -X DELETE "http://localhost:5000/texts/delete/0?password=your_secret_password"
      ```
    - **Response**:
      ```json
        Done
